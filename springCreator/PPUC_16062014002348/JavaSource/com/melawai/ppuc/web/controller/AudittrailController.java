...
public class AudittrailController extends ParentController{
	...
	public String create(@Valid Audittrail audittrail, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
		...
		return "redirect:/master/audittrail/" + encodeUrlPathSegment(audittrail.getId().toString(), httpServletRequest);
	}
	...
	public String show(@PathVariable("id") Long id, Model uiModel) {
		...
		uiModel.addAttribute("audittrail", audittrailManager.get(id));
	...
	public String updateForm(@PathVariable("id") Long id, Model uiModel) {
		populateEditForm(uiModel, audittrailManager.get(id));
		return "audittrail/update";
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE, produces = "text/html")
	public String delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
		Audittrail audittrail = audittrailManager.get(id);
		audittrailManager.remove(id);
		uiModel.asMap().clear();
